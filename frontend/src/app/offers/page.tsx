...
export default async function OffersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  try {
    data = await getOffers({ page, page_size: 20, state, is_active });
  } catch (e) {
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <Link href="/offers" className="text-xs text-red-700 underline mt-2 inline-block">
            Réessayer
          </Link>
        </div>
      )}
      ...
      {!error && data && data.items.length === 0 && (
        <div className="bg-white border border-dashed border-gray-300 rounded-lg p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {state
              ? `Aucune offre dans l'état "${state}".`
              : "Aucune offre en base. Lancez le script de seed."}
          </p>
          {state && (
            <Link href="/offers" className="text-sm text-blue-600 hover:underline mt-3 inline-block">
      ...
      {data && (data.page > 1 || data.has_next) && (
        <div className="flex items-center justify-center gap-4 pt-4">
          {data.page > 1 && (
            <Link
              href={`/offers?page=${data.page - 1}${state ? `&state=${state}` : ""}`}
